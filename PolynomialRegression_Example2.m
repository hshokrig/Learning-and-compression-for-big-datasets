...
load Example2Func.mat
Dataset.Training.size = 100;
Dataset.Test.size = 1000;
Dataset.Training.X = linspace(0,1,Dataset.Training.size);

Dataset.Test.X = linspace(0, 1, Dataset.Test.size);
Dataset.Test.Y = feval(Example2Func, Dataset.Test.X)';
...
%% Iterative algorithm
Plot_variable = 1;
K = fliplr(round(logspace(log10(25),log10(100),12)));
MonteCarlo = 500;
...
for MonteCarlo_index = 1:MonteCarlo
    for K_index = 1:length(K)
        ...
        % Noise = 0;
        Dataset.Training.Y = feval(Example2Func, Dataset.Training.X)' + Noise;
...
save(sprintf('Results/PolynomialRegression_Example2.mat'))
