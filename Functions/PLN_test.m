function [train_error, test_error, train_accuracy, test_accuracy, Total_NN_size, NumNode_opt,new_img,t_hati_test]=...
    PLN(X_train, T_train, X_test, T_test, g, NumNodes, eps_o, mu, kmax, lam, eta_n, eta_l,...
    First_Block,Index,Kappa,reconstruction_layer,Num_regen)
%%  Name:   PLN
%
%   Inputs:
    ...
    if Thr_l==1
        ...
        while i<size(NumNodes,2)
            ...
            if Thr_n==1
                ...
                if layer==reconstruction_layer
                    [r_hat,X_rhat]=my_func(t_hati,T_train,X_train,Index);
                    [r_hat_orth,a_hat,new_img]=my_func2(r_hat,Kappa,Index,X_rhat,Num_regen);
                end
